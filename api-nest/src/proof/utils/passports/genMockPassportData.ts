...
export function genMockPassportData(
  ...
  if (birthDate.length !== 6 || expiryDate.length !== 6) {
    throw new Error(
      'birthdate and expiry date have to be in the "YYMMDD" format',
    );
  }
  ...
  const signedAttr = generateSignedAttr(
    hash(eContentHashAlgo, eContent) as number[],
  );
  const hashAlgo = signatureType.split('_')[1];
  const signature = sign(privateKeyPem, dsc, hashAlgo, signedAttr);
  const signatureBytes = Array.from(signature, (byte) =>
    byte < 128 ? byte : byte - 256,
  );
...
function sign(
  ...
  if (signatureAlgorithm === 'rsapss') {
    ...
    const pss = forge.pss.create({
      ...
      saltLength: parseInt(
        (publicKeyDetails as PublicKeyDetailsRSAPSS).saltLength,
      ),
    });
    ...
    const privateKeyBuffer = (asn1Data.result.valueBlock as any).value[1]
