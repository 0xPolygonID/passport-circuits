...
export function initPassportDataParsing(passportData: PassportData) {
  ...
  if (passportData.passportMetadata.csca) {
    const cscaParsed = parseCertificateSimple(
      passportData.passportMetadata.csca,
    );
...
export function formatSignatureDSCCircuit(
  ...
  const { n, k } = getNAndK(
    cscaSignatureAlgorithmFullName as SignatureAlgorithm,
  );
  if (cscaSignatureAlgorithm === 'ecdsa') {
    ...
    return formatInput(
      splitToWords(BigInt(bytesToBigDecimal(signature)), n, k),
    );
  }
}
...
export function getPubKeyBytes(
  passportData: PassportData,
  type: 'dsc' | 'csca',
): number[] {
  if (type === 'dsc') {
...
function getRsaPubKeyBytes(parsedCertificate: any): number[] {
  const pubKeyHex = (parsedCertificate.publicKeyDetails as PublicKeyDetailsRSA)
    .modulus;
...
export function extractRSFromSignature(signatureBytes: number[]): {
  r: string;
  s: string;
} {
