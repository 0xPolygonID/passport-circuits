...
async function prepareTestData(mrz: string, lastNameSize: number, firstNameSize: number) {
  ...
  const updateTemplate = [
    ...
    '8713837106709436881047310678745516714551061952618778897121563913918335939585', '1742578132201289000', // issuanceDate.id
  ...
  return {
    ...
    issuanceDate: "1742578132201289000",
...
testSuite.forEach(({ shaAlg, shaLength }) => {
  describe(`credential_${shaAlg}.circom`, function () {
    ...
    it(`Passport is valid and hashAlg is ${shaAlg}`, async function () {
      ...
      // Hash Index
      assert(
        w[10] === 14556845464219268658794157901992238391581270841978987851544993150892525062114n
      );
