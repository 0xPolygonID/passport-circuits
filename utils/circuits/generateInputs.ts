...
import { formatMrzTD1, formatMrzTD3 } from '../passports/format';
...
export async function generateCircuitInputsCredential(passportData: PassportData, opts?: Options) {
  const mrzByteArray = formatMrzTD3(passportData.mrz);
...
export async function generateCircuitInputsIdCard(passportData: PassportData, opts?: Options) {
  const mrzByteArray = formatMrzTD1(passportData.mrz);
  if (mrzByteArray.length !== 95) {
    throw new Error('TD1 MRZ should be 95 bytes long');
  }
  ...
  const updateTemplate = [
    ...
    '3286800018689036072036595048281161368331306321215602580795106602635276597696', // credentialSubject.governmentIdentifier
    '12037662945351652395520680282306597407040165994104304811455681806232413956620',
    '17727255306830187653590286089457356657690171618833417839353404673521622315049', // credentialSubject.governmentIdentifierType
    '16829829523990922339853122033176330960757159233571217495904710638791793740933',
    '4366613503740245542741816499068547859478657796760861141829344679607332353738', // credentialSubject.sex
    '18652354674254268839450839640508993614932212252620036777561285260846450401086',
    '0', // credentialStatus.revocationNonce
  ...
  // For TD1, the name is on line 3 (positions 60-89)
  // Format: LASTNAME<<FIRSTNAME
  const nameLine = passportData.mrz.substring(60, 90);
  const lastName = nameLine.split('<<')[0];
  const firstName = nameLine.split('<<')[1]?.replace(/<+$/, '') || '';
  const holderNameSize = lastName.length + firstName.length + 2; // 2 for the double space
