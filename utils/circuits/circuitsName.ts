...
export function getCircuitNameFromPassportData(passportData: PassportData, circuitType: 'register' | 'dsc' | 'signature') {
  if (circuitType === 'register') {
    return getRegisterNameFromPassportData(passportData);
  } else if (circuitType === 'signature') {
    return getSignatureNameFromPassportData(passportData);
  } else {
...
function getSignatureNameFromPassportData(passportData: PassportData) {
  console.log('Getting signature circuit name from passport data...');
  ...
  if (parsedDsc.signatureAlgorithm === 'ecdsa') {
    ...
    console.log('ECDSA curve:', curveOrExponent);
    const circuitName = `signature_${dgHashAlgo}_${eContentHashAlgo}_${signedAttrHashAlgo}_${sigAlg}_${curveOrExponent}`;
    console.log('Generated circuit name:', circuitName);
    return circuitName;
    ...
    if (signatureAlgorithmBits <= 4096) {
      const circuitName = `signature_${dgHashAlgo}_${eContentHashAlgo}_${signedAttrHashAlgo}_${sigAlg}_${curveOrExponent}_${4096}`;
      console.log('Generated circuit name:', circuitName);
      return circuitName;
    } else {
      console.error('RSA key length exceeds maximum supported length');
    ...
    if (signatureAlgorithmBits <= 4096) {
      const circuitName = `signature_${dgHashAlgo}_${eContentHashAlgo}_${signedAttrHashAlgo}_${sigAlg}_${curveOrExponent}_${saltLength}_${signatureAlgorithmBits}`;
      console.log('Generated circuit name:', circuitName);
      return circuitName;
    } else {
      console.error('RSA-PSS key length exceeds maximum supported length');
