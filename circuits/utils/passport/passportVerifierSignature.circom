...
/// @title PassportVerifierSignature
...
template PassportVerifierSignature(ECONTENT_HASH_ALGO, signatureAlgorithm, n, k, MAX_ECONTENT_PADDED_LEN, MAX_SIGNED_ATTR_PADDED_LEN) {
    ...
    */
    ...
    /* - [dg1_removed] DG1 hash calculation and check
    // compute hash of DG1
    ...
    */
    ...
    This part verifies that the hash of eContent (computed in eContentShaBytes) matches the one present in signed_attr. This ensures that:
        ...
        1. eContent has not been altered since it was signed.
        2. The signed attributes (signed_attr) properly reference the original eContent, making sure the signature applies to the expected data.
    */
